...
public class Spiny extends BaseEntity {
	...
	private Animation spinyWalkLeft, spinyWalkRight;
	
	public Spiny(BaseEntityId id, int x, int y, BaseEntityHandler baseEntityHandler) {
		super(id, x, y);	
		this.baseEntityHandler = baseEntityHandler;	
		spinyWalkLeft = new Animation(SPEED, texture.spiny[0], texture.spiny[1]);
		spinyWalkRight = new Animation(SPEED, texture.spiny[2], texture.spiny[3]);
		velocityX = 1;
	}
	...
	public void render(Graphics g) {	
		if(velocityX != 0) {
			if(direction == 1) {
				spinyWalkRight.drawAnimation(g,(int)x, (int)y, 38, 38);
			} else {
				spinyWalkLeft.drawAnimation(g, (int)x, (int)y, 38, 38);
			}
		}
	}
	...
	public void movement(ArrayList<BaseEntity> baseEntities) {
		...
		spinyWalkLeft.runAnimation();
		spinyWalkRight.runAnimation();
	}
