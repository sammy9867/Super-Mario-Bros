...
public class BuzzyBeetle extends BaseEntity {
	...
	private Animation buzzyBeetleWalkLeft, buzzyBeetleWalkRight;
	
	public BuzzyBeetle(BaseEntityId id, int x, int y, BaseEntityHandler baseEntityHandler) {
		super(id, x, y);	
		this.baseEntityHandler = baseEntityHandler;	
		buzzyBeetleWalkLeft = new Animation(SPEED, texture.buzzyBeetle[0], texture.buzzyBeetle[1]);
		buzzyBeetleWalkRight = new Animation(SPEED, texture.buzzyBeetle[2], texture.buzzyBeetle[3]);
		velocityX = 1;
	}
	...
	public void render(Graphics g) {	
		if(velocityX != 0) {
			if(direction == 1) {
				buzzyBeetleWalkRight.drawAnimation(g,(int)x, (int)y, 38, 38);
			} else {
				buzzyBeetleWalkLeft.drawAnimation(g, (int)x, (int)y, 38, 38);
			}
		}
	}
	...
	public void movement(ArrayList<BaseEntity> baseEntities) {
		...
		buzzyBeetleWalkLeft.runAnimation();
		buzzyBeetleWalkRight.runAnimation();
	}
